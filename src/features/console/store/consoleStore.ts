...
function getLocalCommandOutput(command: string) {
  ...
  return [
    ...
    "Backend terminal execution is not connected yet.",
  ].join("\n");
}
...
export const useConsoleStore = create<ConsoleState>((set, get) => ({
  entries: [
    createEntry({
      type: "system",
      content:
        "Console ready. Local shell UI is active. Backend terminal execution is pending.",
    }),
  ],

  commands: [],
  history: [],
  currentWorkingDirectory: "/workspace/frontend-ui",
  isRunning: false,
  ...
  runCommand: async (command) => {
    ...
    set((state) => ({
      ...
      history:
        state.history.at(-1) === cleanCommand
          ? state.history
          : [...state.history, cleanCommand],
      entries: [
        ...state.entries,
        createEntry({
          type: "input",
          content: cleanCommand,
          commandId: consoleCommand.id,
        }),
      ],
    }));
