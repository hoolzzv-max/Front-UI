...
export class LocalStorageDriver {
  ...
  get<T>(key: string, fallback?: T): T | null {
    ...
    try {
      ...
      throw new AppError(`Failed to read localStorage key "${key}".`, {
  ...
  set<T extends StorageValue>(key: string, value: T) {
    ...
    try {
      ...
      throw new AppError(`Failed to write localStorage key "${key}".`, {
  ...
  remove(key: string) {
    ...
    try {
      ...
      throw new AppError(`Failed to remove localStorage key "${key}".`, {
  ...
  has(key: string) {
    ...
    return window.localStorage.getItem(buildKey(this.namespace, key)) !== null;
  }
