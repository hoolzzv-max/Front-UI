...
export class EventBus<Events extends EventMap = EventMap> {
  ...
  on<EventName extends keyof Events>(eventName: EventName, handler: EventHandler<Events[EventName]>): EventSubscription {
  ...
  once<EventName extends keyof Events>(eventName: EventName, handler: EventHandler<Events[EventName]>): EventSubscription {
    const unsubscribe = this.on(eventName, async (payload) => { unsubscribe(); await handler(payload); });
    return unsubscribe;
  }

  off<EventName extends keyof Events>(eventName: EventName, handler: EventHandler<Events[EventName]>) {
  ...
  async emit<EventName extends keyof Events>(eventName: EventName, payload: Events[EventName]) {
    const handlers = this.listeners.get(eventName);
    if (!handlers || handlers.size === 0) return;
    const errors: AppError[] = [];
    for (const handler of handlers) {
      try { await handler(payload as Events[keyof Events]); }
      catch (error) {
        errors.push(new AppError(`Event handler failed for "${String(eventName)}".`, { code: ERROR_CODES.EVENT_HANDLER_ERROR, cause: error, details: { eventName, payload } }));
      }
    }
    if (errors.length > 0) {
      throw new AppError(`One or more handlers failed for "${String(eventName)}".`, { code: ERROR_CODES.EVENT_HANDLER_ERROR, details: errors.map((error) => error.toJSON()) });
    }
  }

  clear<EventName extends keyof Events>(eventName?: EventName) {
    if (eventName) { this.listeners.delete(eventName); return; }
...
export type WorkspaceEventMap = {
  ...
  "agent:status": { status: string; url?: string };
  ...
  "websocket:status": { status: "idle" | "connecting" | "connected" | "reconnecting" | "disconnected" | "error"; url?: string };
